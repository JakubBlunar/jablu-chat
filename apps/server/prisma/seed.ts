...
const CHANNEL_NAMES = [
  'random', 'dev-talk', 'memes', 'off-topic',
  'music', 'gaming', 'announcements', 'help',
  'design', 'feedback',
];
...
async function main() {
  ...
  if (existingUsers.length > 0) {
    ...
    for (let i = 0; i < USERS; i++) {
      let username: string;
      do {
        username = faker.internet.username().replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 28);
  ...
  const targetBytesPerFile = TARGET_SIZE_MB > 0
    ? Math.round((TARGET_SIZE_MB * 1024 * 1024) / Math.max(1, totalAttachments))
    : 0;
  ...
  const sortedDates = Array.from({ length: MESSAGES }, () => randomDate(30))
  ...
  for (let i = 0; i < MESSAGES; i++) {
    ...
    const msg = await prisma.message.create({
      data: {
        ...
        authorId: author.id,
        content: hasAttachment && Math.random() < 0.3
          ? null
  ...
  console.log(`  Done: ${MESSAGES} messages, ${attachmentsCreated} attachments, ${(totalDiskBytes / (1024 * 1024)).toFixed(1)} MB on disk.`);
  ...
  if (dmPool.length < 2) {
    ...
    for (const [userA, userB] of pairs) {
      ...
      const dmDates = Array.from({ length: DM_MSGS }, () => randomDate(30))
        .sort((a, b) => a.getTime() - b.getTime());
