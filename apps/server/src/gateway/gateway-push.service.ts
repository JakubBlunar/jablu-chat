...
export async function getChannelNotifPrefs(
  ...
  try {
    const cached = await redis.client.hgetall(cacheKey)
  ...
  const prefs = await prisma.channelNotifPref.findMany({
    where: { channelId }
  })
  const map = new Map<string, string>()
  if (prefs.length > 0) {
    ...
    try {
      await redis.client.hmset(cacheKey, hash)
...
export async function sendPushToOfflineMembers(
  ...
  const offlineIds = members.filter((m) => !isUserOnline(m.userId))
